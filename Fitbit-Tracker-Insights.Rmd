...
dfh_o <- dfh %>%
  group_by (Day, Time) %>% 
  summarize(AverageSteps = mean(StepTotal), AverageIntensity = mean(TotalIntensity))
...
```
Viewing the step data, users are typically most active around midday on the weekends and late afternoon / early evening on weekdays. Intensity data essentially mirrors the step data, meaning the users' intense activities are typically movement-based. Now grouping users based on activity level.

```{r}
# creating grouped user data frame
u_dfn <- dfn %>%
  group_by (Id) %>% 
  ...
  # categorizing users by activity level (steps)
...
```
```{r}
# summarizing individual activity data
activity <- u_dfn %>%
